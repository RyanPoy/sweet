...
class RecordMethodMissingTestCase(unittest.TestCase):
    ...
    def test_find_by(self, Criteria):
        Criteria.is_callable().returns_fake()\
                ...
                .expects('where').with_args(age=10, name='poy').returns_fake()\
    ...
    def test_find_all_by(self, Criteria):
        Criteria.is_callable().returns_fake()\
                .expects('set_record_class').with_args(OrmRecord).returns_fake()\
                .expects('where').with_args(age=10, name='poy').returns_fake()\
                .expects('all').returns([{'id': 1, 'name':'poy', 'age':100}, {'id': 10, 'name':'poy', 'age':100}])
    ...
    def test_find_or_init_by_should_be_call_init_if_can_not_found_it(self, Criteria):
        ...
        relt = OrmRecord.find_or_init_by_age_and_name(10, 'poy')
        self.assertIsInstance(relt, OrmRecord)
        self.assertEqual('poy', relt.name)
    ...
    def test_find_or_create_by_should_be_call_create_if_can_not_found_it(self, Criteria):
        c = Criteria.is_callable().returns_fake()\
                .expects('set_record_class').with_args(OrmRecord).returns_fake()
        c.expects('where').with_args(age=10, name='poy').returns_fake()\
         .expects('first').returns(None)
        c.expects('insert').with_args(age=10, name='poy').returns(True)

        relt = OrmRecord.find_or_create_by_age_and_name(10, 'poy')
        self.assertIsInstance(relt, OrmRecord)
        self.assertEqual('poy', relt.name)
        self.assertEqual(10, relt.age)
