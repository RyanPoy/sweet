...
@pytest.mark.asyncio
async def test_mysql_driver__execute_rowid_in_with_block(mysql_env):
...
@pytest.mark.asyncio
async def test_sqlite_driver__execute_rowid_in_with_block(sqlite_env):
    async with await sqlite_env.db.get_connection() as conn:
        id = await conn.execute_rowid("INSERT INTO \"users\" (name) VALUES (?)", "test_name_1")
        assert id == 1
...
@pytest.mark.asyncio
async def test_pg_driver__execute_rowid_in_with_block(pg_env):
    async with await pg_env.db.get_connection() as conn:
        id = await conn.execute_rowid("INSERT INTO \"users\" (name) VALUES ($1) RETURNING id", "test_name_1")
        assert id == 1
