...
class TestPostgreSQLDriver(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        self.driver = await init_db(DB_TYPE.pg)
    ...
    async def test_insert_and_select(self):
        """测试插入和查询功能."""
        await self.driver.execute_rowcount("INSERT INTO \"foos\" (name, code) VALUES ($1, $2)", "test_name_1", 1001)
    ...
    async def test_transaction_commit(self):
        ...
        async with self.driver.transaction():
            await self.driver.execute("INSERT INTO \"foos\" (name, code) VALUES ($1, $2)", "test_name_1", 1001)
            await self.driver.execute("INSERT INTO \"foos\" (name, code) VALUES ($1, $2)", "test_name_2", 1002)
    ...
    async def test_transaction_rollback(self):
        ...
        try:
            async with self.driver.transaction():
                await self.driver.execute("INSERT INTO \"foos\" (name, code) VALUES ($1, $2)", "rollback_test", 3001)
                raise Exception("Rollback this transaction")
        except:
            pass  # 模拟回滚

        result = await self.driver.fetchone("SELECT * FROM \"foos\" WHERE name = $1", "rollback_test")
