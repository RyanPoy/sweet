...
import unittest
from tests.integration.for_sqlite.helper import User
from sweet.record.recordset import SQLiteRecordset, SQLError
from sweet.utils import mydict


class TestRecordsetCRUDSQLite(unittest.TestCase):
    ...
    def test_table(self):
        ...
        self.assertTrue(isinstance(table, SQLiteRecordset))
    ...
    #     u = self.database.records('users').where(age=25).first()
    #     self.assertTrue(u is not None)

    #     r = self.database.records('users').join('mobiles', on="users.id=mobiles.user_id").where(mobiles__name='xiaomi').update(age=52)
    #     self.assertEqual(1, r)
    
    #     u = self.database.records('users').where(age=25).first()
    #     self.assertTrue(u is None)

    #     u = self.database.records('users').where(age=52).first()
    #     self.assertTrue(u is not None)
...
if __name__ == '__main__':
    ...
    # sql = "SELECT `users`.* FROM `users` INNER JOIN `mobiles` ON `users`.`id` = `mobiles`.`user_id` WHERE `mobiles`.`name` = ?"
    # params = [ 'iphone' ]
    # import sqlite3
    # db_args = User.database._db_args
    # conn = sqlite3.connect(**db_args)
