...
class MySQLTableTest(TestCase):
    ...
    def test_where_in_empty(self):
        tb = self.get_table()
        tb.select('*').where(id=[], name='ryanpoy')
    ...
    def test_where_gt(self):
        tb = self.get_table()
        tb.select('*').where(id=[1, 2, 3], age__gt=30)
    ...
    def test_where_gte(self):
        tb = self.get_table()
        tb.select('*').where(id=[1, 2, 3], age__gte=30)
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` >= %s', tb.sql)
        self.assertEqual([1, 2, 3, 30], tb.bindings)

    def test_where_lt(self):
        tb = self.get_table()
        tb.select('*').where(id=[1, 2, 3], age__lt=30)
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` < %s', tb.sql)
        self.assertEqual([1, 2, 3, 30], tb.bindings)

    def test_where_lte(self):
        tb = self.get_table()
        tb.select('*').where(id=[1, 2, 3], age__lte=30)
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` <= %s', tb.sql)
        self.assertEqual([1, 2, 3, 30], tb.bindings)

    def test_where_between(self):
        tb = self.get_table()
        tb.select('*').where(id__bt=[1, 2])
