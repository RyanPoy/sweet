...
class MySQLTableQueryTest(TestCase):
    ...
    def test_default_select(self):
        tb = self.get_table()
        tb = self.assertEqual('SELECT * FROM `users`', tb.sql)

    def test_multi_select(self):
        tb = self.get_table()
        tb = tb.select('id', 'name')
        self.assertEqual('SELECT `id`, `name` FROM `users`', tb.sql)

    def test_select_twice(self):
        tb = self.get_table()
        tb = tb.select('id').select('name')
        self.assertEqual('SELECT `id`, `name` FROM `users`', tb.sql)
    ...
    def test_where_in(self):
        tb = self.get_table()
        tb = tb.select('*').where(id=[1, 2, 3], name='ryanpoy')
    ...
    def test_where_in_empty(self):
        tb = self.get_table()
        tb = tb.select('*').where(id=[], name='ryanpoy')
    ...
    def test_where_gt(self):
        tb = self.get_table()
        tb = tb.select('*').where(id=[1, 2, 3], age__gt=30)
    ...
    def test_where_gte(self):
        tb = self.get_table()
        tb = tb.select('*').where(id=[1, 2, 3], age__gte=30)
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` >= %s', tb.sql)
        self.assertEqual([1, 2, 3, 30], tb.bindings)

    def test_where_lt(self):
        tb = self.get_table()
        tb = tb.select('*').where(id=[1, 2, 3], age__lt=30)
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` < %s', tb.sql)
        self.assertEqual([1, 2, 3, 30], tb.bindings)

    def test_where_lte(self):
        tb = self.get_table()
        tb = tb.select('*').where(id=[1, 2, 3], age__lte=30)
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` <= %s', tb.sql)
        self.assertEqual([1, 2, 3, 30], tb.bindings)

    def test_where_between(self):
        tb = self.get_table()
        tb = tb.select('*').where(id__bt=[1, 2])
    ...
    def test_left_join(self):
        tb = self.get_table()
        tb = tb.select('users.id').select('users.name').select('cars.name').where(id=[1,2,3]).or_(name="ryanpoy").left_join('cars', on="users.id=cars.user_id")
    ...
    def test_right_join(self):
        tb = self.get_table()
        tb = tb.select('users.id').select('users.name').select('cars.name').where(id=[1,2,3]).or_(name="ryanpoy").right_join('cars', on="users.id=cars.user_id")
