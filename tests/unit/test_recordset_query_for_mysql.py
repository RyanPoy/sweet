...
class TestRecordsetQueryForMySQL(TestCase):
    ...
    def test_copy_deep(self):
        ...
        sql, bindings = tb.select('id').select('name').sql()
    ...
    def test_basic_select(self):
        sql, bindings = self.get_recordset().select('*').sql()
        self.assertEqual('SELECT * FROM `users`', sql)
        self.assertEqual([], bindings)

    def test_basic_distinct(self):
        sql, bindings = self.get_recordset().distinct().select('id').sql()
        self.assertEqual('SELECT DISTINCT `id` FROM `users`', sql)
        self.assertEqual([], bindings)

    def test_default_select(self):
        sql, bindings = self.get_recordset().sql()
        self.assertEqual('SELECT * FROM `users`', sql)
        self.assertEqual([], bindings)

    def test_multi_select(self):
        sql, bindings = self.get_recordset().select('id', 'name').sql()
        self.assertEqual('SELECT `id`, `name` FROM `users`', sql)
        self.assertEqual([], bindings)

    def test_select_twice(self):
        sql, bindings = self.get_recordset().select('id').select('name').sql()
        self.assertEqual('SELECT `id`, `name` FROM `users`', sql)
        self.assertEqual([], bindings)

    def test_where(self):
        sql, bindings = self.get_recordset().select('*').where(id=1, name='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` = %s AND `name` = %s', sql)
        self.assertEqual([1, 'ryanpoy'], bindings)

    def test_where_not(self):
        sql, bindings = self.get_recordset().select('*').where(id__not=1, name__not='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` <> %s AND `name` <> %s', sql)
        self.assertEqual([1, 'ryanpoy'], bindings)

    def test_where_in(self):
        sql, bindings = self.get_recordset().select('*').where(id=[1, 2, 3], name='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `name` = %s', sql)
        self.assertEqual([1, 2, 3, 'ryanpoy'], bindings)

    def test_where_not_in(self):
        sql, bindings = self.get_recordset().select('*').where(id__not=[1, 2, 3], name__not='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` NOT IN (%s, %s, %s) AND `name` <> %s', sql)
        self.assertEqual([1, 2, 3, 'ryanpoy'], bindings)

    def test_where_like(self):
        sql, bindings = self.get_recordset().select('*').where(id=[1, 2, 3], name__like='%yanpo%').sql()
    ...
    def test_where_not_like(self):
        sql, bindings = self.get_recordset().select('*').where(id=[1, 2, 3], name__not_like='%yanpo%').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `name` NOT LIKE %s', sql)
        self.assertEqual([1, 2, 3, '%yanpo%'], bindings)

    def test_where_in_empty(self):
        sql, bindings = self.get_recordset().select('*').where(id=[], name='ryanpoy').sql()
    ...
    def test_where_in_none(self):
        sql, bindings = self.get_recordset().select('*').where(id=[None, None], name='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s) AND `name` = %s', sql)
        self.assertEqual([None, None, 'ryanpoy'], bindings)

    def test_where_null(self):
        sql, bindings = self.get_recordset().select('*').where(id=[1, 2, 3], name=None).sql()
    ...
    def test_where_not_null(self):
        sql, bindings = self.get_recordset().select('*').where(id=[1, 2, 3]).where(name__not=None).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `name` IS NOT NULL', sql)
        self.assertEqual([1, 2, 3], bindings)

    def test_where_gt(self):
        sql, bindings = self.get_recordset().select('*').where(id=[1, 2, 3], age__gt=30).sql()
    ...
    def test_where_gte(self):
        sql, bindings = self.get_recordset().select('*').where(id=[1, 2, 3], age__gte=30).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` >= %s', sql)
        self.assertEqual([1, 2, 3, 30], bindings)

    def test_where_lt(self):
        sql, bindings = self.get_recordset().select('*').where(id=[1, 2, 3], age__lt=30).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` < %s', sql)
        self.assertEqual([1, 2, 3, 30], bindings)

    def test_where_lte(self):
        sql, bindings = self.get_recordset().select('*').where(id=[1, 2, 3], age__lte=30).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` <= %s', sql)
        self.assertEqual([1, 2, 3, 30], bindings)

    def test_where_between(self):
        sql, bindings = self.get_recordset().select('*').where(id__bt=[1, 2]).sql()
    ...
    def test_where_not_between(self):
        sql, bindings = self.get_recordset().select('*').where(id__not_bt=[1, 2]).sql()
    ...
    def test_where_group_parameters(self):
        sql, bindings = self.get_recordset().select('*').where(id__bt=[1, 2]).or_where(
        ...
        self.assertEqual([1, 2, 'jim', 'lucy', 10], bindings)

    def test_or(self):
        sql, bindings = self.get_recordset().select('*').or_where(id=1, name='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` = %s OR `name` = %s', sql)
        self.assertEqual([1, 'ryanpoy'], bindings)

    def test_or_not(self):
        sql, bindings = self.get_recordset().select('*').or_where(id__not=1, name__not='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` <> %s OR `name` <> %s', sql)
        self.assertEqual([1, 'ryanpoy'], bindings)

    def test_or_between(self):
        sql, bindings = self.get_recordset().select('*').where(name='ryanpoy').or_where(id__bt=[1, 2]).sql()
    ...
    def test_or_not_between(self):
        sql, bindings = self.get_recordset().select('*').where(name='ryanpoy').or_where(id__not_bt=[1, 2]).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `name` = %s OR `id` NOT BETWEEN %s AND %s', sql)
        self.assertEqual(['ryanpoy', 1, 2], bindings)

    def test_or_in(self):
        sql, bindings = self.get_recordset().select('*').or_where(id=[1, 2, 3], name='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) OR `name` = %s', sql)
        self.assertEqual([1, 2, 3, 'ryanpoy'], bindings)

    def test_or_not_in(self):
        sql, bindings = self.get_recordset().select('*').or_where(id__not=[1, 2, 3], name__not='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` NOT IN (%s, %s, %s) OR `name` <> %s', sql)
        self.assertEqual([1, 2, 3, 'ryanpoy'], bindings)
    
    def test_or_null(self):
        sql, bindings = self.get_recordset().select('*').or_where(id=[1, 2, 3], name=None).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) OR `name` IS NULL', sql)
        self.assertEqual([1, 2, 3], bindings)

    def test_or_not_null(self):
        sql, bindings = self.get_recordset().select('*').or_where(id=[1, 2, 3]).or_where(name__not=None).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) OR `name` IS NOT NULL', sql)
        self.assertEqual([1, 2, 3], bindings)
    
    def test_order_by(self):
        sql, bindings = self.get_recordset().select('*').order_by('email').order_by('age', 'desc').sql()
        self.assertEqual('SELECT * FROM `users` ORDER BY `email`, `age` DESC', sql)
        self.assertEqual([], bindings)

    def test_group_bys(self):
        sql, bindings = self.get_recordset().select('*').group_by('id', 'email').sql()
        self.assertEqual('SELECT * FROM `users` GROUP BY `id`, `email`', sql)
        self.assertEqual([], bindings)

    def test_where_group_parameters(self):
        sql, bindings = self.get_recordset().select('*').having(id=1, name='ryanpoy').or_having(
            WhereClause('`', '%s').and_(name='jim').or_(name='lucy')
        ).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` = %s AND `name` = %s OR ( `name` = %s OR `name` = %s )', sql)
        self.assertEqual([1, 'ryanpoy', 'jim', 'lucy'], bindings)

    def test_having(self):
        sql, bindings = self.get_recordset().select('*').having(id=1, name='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` = %s AND `name` = %s', sql)
        self.assertEqual([1, 'ryanpoy'], bindings)

    def test_having_in(self):
        sql, bindings = self.get_recordset().select('*').having(id=[1, 2, 3], name='ryanpoy').sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `name` = %s', sql)
        self.assertEqual([1, 2, 3, 'ryanpoy'], bindings)

    def test_having_null(self):
        sql, bindings = self.get_recordset().select('*').having(id=[1, 2, 3], name=None).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `name` IS NULL', sql)
        self.assertEqual([1, 2, 3], bindings)

    def test_having_gt(self):
        sql, bindings = self.get_recordset().select('*').having(id=[1, 2, 3], age__gt=30).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `age` > %s', sql)
        self.assertEqual([1, 2, 3, 30], bindings)

    def test_having_gte(self):
        sql, bindings = self.get_recordset().select('*').having(id=[1, 2, 3], age__gte=30).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `age` >= %s', sql)
        self.assertEqual([1, 2, 3, 30], bindings)

    def test_having_lt(self):
        sql, bindings = self.get_recordset().select('*').having(id=[1, 2, 3], age__lt=30).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `age` < %s', sql)
        self.assertEqual([1, 2, 3, 30], bindings)

    def test_having_lte(self):
        sql, bindings = self.get_recordset().select('*').having(id=[1, 2, 3], age__lte=30).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `age` <= %s', sql)
        self.assertEqual([1, 2, 3, 30], bindings)

    def test_having_between(self):
        sql, bindings = self.get_recordset().select('*').having(id__bt=[1, 2]).sql()
    ...
    def test_join(self):
        ...
        self.assertEqual([ 1, 2, 3, 'ryanpoy', 'focus'], bindings)

    def test_join_without_select(self):
        sql, bindings = self.get_recordset().where(id=[1,2,3]).or_where(name="ryanpoy").join('cars', "users.id=cars.user_id").where(cars__name='focus').sql()
    ...
    def test_join_with_function(self):
        ...
        sql, bindings = self.get_recordset().join('cars', func=complex).sql()

        self.assertEqual('SELECT * FROM `users` INNER JOIN `cars` ON `users`.`id` = `cars`.`user_id` AND `cars`.`id` = %s OR `users`.`id` = %s', sql)
        self.assertEqual([10, 100], bindings)

    def test_left_join(self):
        sql, bindings = self.get_recordset().select('users.id').select('users.name').select('cars.name').where(id=[1,2,3]).or_where(name="ryanpoy").left_join('cars', "users.id=cars.user_id").sql()
    ...
    def test_right_join(self):
        sql, bindings = self.get_recordset().select('users.id').select('users.name').select('cars.name').where(id=[1,2,3]).or_where(name="ryanpoy").right_join('cars', "users.id=cars.user_id").sql()
    ...
    def test_cross_join(self):
        sql, bindings = self.get_recordset().select('users.id').select('users.name').select('cars.name').where(id=[1,2,3]).or_where(name="ryanpoy").cross_join('cars', "users.id=cars.user_id").sql()
    ...
    def test_read_lock(self):
        sql, bindings = self.get_recordset().select('users.id').select('cars.name')\
    ...
    def test_multiple_where_exists(self):
        ...
        self.assertEqual(['iphone', 'aphone'], bindings)

    def test_or_exists(self):
        users = self.get_recordset()
        mobiles = self.get_recordset("mobiles")

        sql, bindings = users.where(age__lte=20).or_exists(
            mobiles.where(name='iphone')
        ).sql()
    ...
    def test_multiple_or_exists(self):
        sql, bindings = self.get_recordset().or_exists(
            self.get_recordset("mobiles").where(name='iphone'),
            self.get_recordset("mobiles").where(name='aphone')
        ).sql()
    ...
    def test_complex_where_or_exists(self):
        sql, bindings = self.get_recordset().where(age__lte=20).where_exists(
                    self.get_recordset("mobiles").where(name='iphone'),
                ).or_exists(
                    self.get_recordset("mobiles").where(name='iphone'),
                    self.get_recordset("mobiles").where(name='aphone')
        ...
        self.assertEqual([20, 'iphone', 'iphone', 'aphone', 'aphone'], bindings)

    def test_unions(self):
        sql, bindings = self.get_recordset().where(id=1).union(
    ...
    def test_union_alls(self):
        sql, bindings = self.get_recordset().where(id=1).union_all(
            self.get_recordset().where(id=2)
        ).sql()
    ...
    def test_union_order_by_and_limits_and_offsets(self):
        sql, bindings = self.get_recordset().where(id=1).union(
            self.get_recordset().where(id=2)
        ).limit(10).offset(20).order_by('id', False).sql()
        self.assertEqual(
            'SELECT * FROM `users` WHERE `id` = %s '
            'UNION SELECT * FROM `users` WHERE `id` = %s '
