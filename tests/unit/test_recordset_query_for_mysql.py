...
class TestRecordsetQueryForMySQL(TestCase):
    ...
    def test_copy_deep(self):
        ...
        sql, params = tb.select('id').select('name').sql()
    ...
    def test_basic_distinct(self):
        sql, params = self.get_recordset().distinct().select('id').sql()
        self.assertEqual('SELECT DISTINCT `id` FROM `users`', sql)
        self.assertEqual([], params)

    def test_default_select(self):
        sql, params = self.get_recordset().sql()
        self.assertEqual('SELECT * FROM `users`', sql)
        self.assertEqual([], params)

    def test_multi_select(self):
        sql, params = self.get_recordset().select('id', 'name').sql()
        self.assertEqual('SELECT `id`, `name` FROM `users`', sql)
        self.assertEqual([], params)

    def test_select_twice(self):
        sql, params = self.get_recordset().select('id').select('name').sql()
        self.assertEqual('SELECT `id`, `name` FROM `users`', sql)
        self.assertEqual([], params)
    ...
    def test_where_not_like(self):
        sql, params = self.get_recordset().select('*').where(id=[1, 2, 3], name__not_like='%yanpo%').sql()
    ...
    def test_where_not_null(self):
        sql, params = self.get_recordset().select('*').where(id=[1, 2, 3]).where(name__not=None).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `name` IS NOT NULL', sql)
        self.assertEqual([1, 2, 3], params)

    def test_where_gt(self):
        sql, params = self.get_recordset().select('*').where(id=[1, 2, 3], age__gt=30).sql()
    ...
    def test_where_gte(self):
        sql, params = self.get_recordset().select('*').where(id=[1, 2, 3], age__gte=30).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` >= %s', sql)
        self.assertEqual([1, 2, 3, 30], params)

    def test_where_lt(self):
        sql, params = self.get_recordset().select('*').where(id=[1, 2, 3], age__lt=30).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` < %s', sql)
        self.assertEqual([1, 2, 3, 30], params)

    def test_where_lte(self):
        sql, params = self.get_recordset().select('*').where(id=[1, 2, 3], age__lte=30).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) AND `age` <= %s', sql)
        self.assertEqual([1, 2, 3, 30], params)

    def test_where_between(self):
        sql, params = self.get_recordset().select('*').where(id__bt=[1, 2]).sql()
    ...
    def test_where_not_between(self):
        sql, params = self.get_recordset().select('*').where(id__not_bt=[1, 2]).sql()
    ...
    def test_or_not_null(self):
        sql, params = self.get_recordset().select('*').or_where(id=[1, 2, 3]).or_where(name__not=None).sql()
        self.assertEqual('SELECT * FROM `users` WHERE `id` IN (%s, %s, %s) OR `name` IS NOT NULL', sql)
        self.assertEqual([1, 2, 3], params)
    
    def test_order_by(self):
        sql, params = self.get_recordset().select('*').order_by('email').order_by('age', 'desc').sql()
        self.assertEqual('SELECT * FROM `users` ORDER BY `email`, `age` DESC', sql)
        self.assertEqual([], params)

    def test_group_bys(self):
        sql, params = self.get_recordset().select('*').group_by('id', 'email').sql()
    ...
    def test_having_gt(self):
        sql, params = self.get_recordset().select('*').having(id=[1, 2, 3], age__gt=30).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `age` > %s', sql)
        self.assertEqual([1, 2, 3, 30], params)

    def test_having_gte(self):
        sql, params = self.get_recordset().select('*').having(id=[1, 2, 3], age__gte=30).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `age` >= %s', sql)
        self.assertEqual([1, 2, 3, 30], params)

    def test_having_lt(self):
        sql, params = self.get_recordset().select('*').having(id=[1, 2, 3], age__lt=30).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `age` < %s', sql)
        self.assertEqual([1, 2, 3, 30], params)

    def test_having_lte(self):
        sql, params = self.get_recordset().select('*').having(id=[1, 2, 3], age__lte=30).sql()
        self.assertEqual('SELECT * FROM `users` HAVING `id` IN (%s, %s, %s) AND `age` <= %s', sql)
        self.assertEqual([1, 2, 3, 30], params)

    def test_having_between(self):
        sql, params = self.get_recordset().select('*').having(id__bt=[1, 2]).sql()
    ...
    def test_page(self):
        sql, params = self.get_recordset().select('*').page(2, 15).sql()
    ...
    def test_join_with_function(self):
        ...
        sql, params = self.get_recordset().join('cars', func=complex).sql()
